...
**Architecture:** `AttachmentAutoPreviewSettings` struct lives in `JXCODECore`. `AppState` owns and persists it to UserDefaults. The existing `startBridgeObservation` loop pushes the value from `AppState` into each `ChatBridge`. `InputBarView` reads from `chatBridge.autoPreviewSettings` and skips attachment creation for disabled types.
...
|---|---|---|
| `Packages/Sources/JXCODECore/Models/AttachmentAutoPreviewSettings.swift` | Create | Settings model — four Bool fields, Codable |
| `Packages/Sources/JXCODEChatKit/ChatBridge.swift` | Modify | Add `autoPreviewSettings` property for InputBarView to read |
| `JXCODE/App/AppState.swift` | Modify | Add `autoPreviewSettings` property with UserDefaults persistence |
| `JXCODE/App/AppState.swift` (`startBridgeObservation`) | Modify | Push settings into bridge inside observation loop |
| `Packages/Sources/JXCODEChatKit/InputBarView.swift` | Modify | Guard each attachment-creation path with the matching setting |
| `JXCODE/Views/SettingsView.swift` | Modify | Add four Toggles in a new section inside `ChatSettingsTab` |
...
**Files:**
- Create: `Packages/Sources/JXCODECore/Models/AttachmentAutoPreviewSettings.swift`

- [ ] **Step 1: Create the file**

```swift
// Packages/Sources/JXCODECore/Models/AttachmentAutoPreviewSettings.swift
import Foundation
...
- [ ] **Step 2: Build JXCODECore to verify**

```bash
cd /Users/jmlee/workspace/JXCODE/Packages && swift build --target JXCODECore 2>&1 | tail -5
```
...
```bash
git add Packages/Sources/JXCODECore/Models/AttachmentAutoPreviewSettings.swift
...
**Files:**
- Modify: `Packages/Sources/JXCODEChatKit/ChatBridge.swift`

- [ ] **Step 1: Add property to ChatBridge**

Open `Packages/Sources/JXCODEChatKit/ChatBridge.swift`. After the last `public var` in the "Streaming State" section (after `sessionStats`), add:
...
- [ ] **Step 2: Build JXCODEChatKit to verify**

```bash
cd /Users/jmlee/workspace/JXCODE/Packages && swift build --target JXCODEChatKit 2>&1 | tail -5
```
...
```bash
git add Packages/Sources/JXCODEChatKit/ChatBridge.swift
...
Open `JXCODE/App/AppState.swift`. After the `focusMode` property (around line 199), add a new MARK and property:
...
```bash
xcodebuild -project /Users/jmlee/workspace/JXCODE/JXCODE.xcodeproj -scheme JXCODE -configuration Debug build 2>&1 | grep -E "error:|Build complete|BUILD SUCCEEDED|BUILD FAILED" | tail -5
```
...
**Files:**
- Modify: `JXCODE/App/AppState.swift` (the `startBridgeObservation` function, around line 489)

- [ ] **Step 1: Add sync line inside withObservationTracking**

Open `JXCODE/App/AppState.swift`. In `startBridgeObservation`, inside the `withObservationTracking` closure, add one line after `bridge.sessionStats = ...`:
...
```bash
xcodebuild -project /Users/jmlee/workspace/JXCODE/JXCODE.xcodeproj -scheme JXCODE -configuration Debug build 2>&1 | grep -E "error:|BUILD SUCCEEDED|BUILD FAILED" | tail -5
```
...
**Files:**
- Modify: `Packages/Sources/JXCODEChatKit/InputBarView.swift`
...
- [ ] **Step 6: Build JXCODEChatKit to verify**
...
```bash
git add Packages/Sources/JXCODEChatKit/InputBarView.swift
...
**Files:**
- Modify: `JXCODE/Views/SettingsView.swift`

- [ ] **Step 1: Add autoPreviewSection helper**

Open `JXCODE/Views/SettingsView.swift`. Inside `ChatSettingsTab`, after `focusModeSection` (around line 443, before `effortDisplayName`), add a new helper:
...
```bash
git add JXCODE/Views/SettingsView.swift
