...
JXCODE is a native macOS desktop client for the Claude Code CLI. Written in Swift + SwiftUI with two external dependencies: SwiftTerm (terminal emulation) and Sparkle (auto-update).
...
# Release build
xcodebuild -project JXCODE.xcodeproj -scheme JXCODE -configuration Release build
```
...
- **App entry point** (`App/JXCODEApp.swift`): Defines WindowGroup (main), WindowGroup(id: "project-window") for dedicated per-project windows, Settings window, and Command menu (theme, update).
...
| Package | Role |
|---------|------|
| `JXCODECore` | Shared models, theme, utilities — no UI dependencies |
| `JXCODEChatKit` | Chat UI components (ChatView, MessageBubble, InputBarView, etc.) |
...
| `PersistenceService` | JSON file-based persistence at `~/Library/Application Support/JXCODE/`. Per-project/session directory structure |
