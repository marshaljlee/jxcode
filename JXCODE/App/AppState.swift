...
final class AppState {
    ...
    func initialize() async {
        ...
        // JXCODE-owned JSON store, preferring CLI-backed when both exist for
        // the same session id.
        ...
        // Migrate legacy JXCODE JSON sessions to CLI-compatible jsonl so they can
        ...
        let legacySummaries = allSessionSummaries.filter { $0.origin == .legacyJXCODE }
    ...
    func forkFromHere(messageId: UUID, in window: WindowState) async {
        ...
        // `claude --resume` picker, matching how JXCODE-spawned sessions are exposed.
    ...
    /// One-time migration: converts legacy JXCODE JSON sessions to CLI-compatible jsonl files.
    ...
    private func reloadSessionSummaries(for project: Project) async {
        ...
        // owned solely by JXCODE, so for a session already in memory the
        // in-memory value is never staler than disk. This reload only exists to
        // pick up CLI-side jsonl changes, so carry those JXCODE-owned fields over
    ...
    private func releaseOutgoingSession(_ outgoingId: String?, excluding newId: String? = nil, in window: WindowState) {
        ...
        Task { [weak self] in
            ...
            if !outgoingMessages.isEmpty, let project = window.selectedProject {
                ...
                let title = existing?.title ?? "Session"
                // Carry over the JXCODE-owned sidecar fields and status-bar stats so
    ...
    func cloneAndAddProject(_ repo: GitHubRepo, in window: WindowState) async throws {
        ...
        let clonePath = "\(home)/JXCODE/\(repo.name)"
        let parentDir = "\(home)/JXCODE"
        let fm = FileManager.default
    ...
    func addAgent(name: String, icon: String, prompt: String, defaultTask: String?, model: String, fileRead: Bool, fileWrite: Bool, network: Bool, hooks: String?) async {
        let newAgent = Agent(
            name: name,
            icon: icon,
            systemPrompt: prompt,
        ...
        try? await persistence.saveAgents(agents)
    }

    func updateAgent(_ updated: Agent) async {
    ...
    func deleteAgent(id: UUID) async {
        agents.removeAll { $0.id == id }
        try? await persistence.saveAgents(agents)
    }

    func executeAgent(agent: Agent, task: String, projectPath: String) async {
        let runId = UUID()
        let logPath = "agent_runs/\(runId.uuidString).log"
        ...
        try? await persistence.saveAgentRuns(agentRuns)
        
        // Spawn subprocess in background Task
        Task.detached(priority: .background) { [weak self] in
            guard let self = self else { return }
            
            guard let binary = await self.claude.findClaudeBinary() else {
                await self.updateAgentRunStatus(runId: runId, status: "failed", errorMsg: "Claude binary not found")
                return
            }
            
            // Write initial task header to log
            let initialLog = "=== Agent Execution Started ===\nAgent: \(agent.name)\nTask: \(task)\nModel: \(agent.model)\n\n"
            try? await self.persistence.writeAgentRunLog(runId: runId, text: initialLog)
            
            let proc = Process()
            proc.executableURL = URL(fileURLWithPath: binary)
            
            // Build args matching opcode
            let args = [
                "-p", task,
            ...
            env["PATH"] = "/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin:/opt/homebrew/bin:\(NSHomeDirectory())/.local/bin"
            ...
            // Read output stream asynchronously and append to log
            let readTask = Task {
                let fileHandle = outputPipe.fileHandleForReading
                for try await line in fileHandle.bytes.lines {
                    try? await self.persistence.appendAgentRunLog(runId: runId, text: line + "\n")
                }
            }
            
            do {
                try proc.run()
                await self.updateAgentRunPid(runId: runId, pid: proc.processIdentifier)
                
                proc.waitUntilExit()
                readTask.cancel()
                
                let success = proc.terminationStatus == 0
                await self.updateAgentRunStatus(runId: runId, status: success ? "completed" : "failed", errorMsg: success ? nil : "Process exited with code \(proc.terminationStatus)")
            } catch {
                readTask.cancel()
                await self.updateAgentRunStatus(runId: runId, status: "failed", errorMsg: error.localizedDescription)
            }
        }
    }

    private func updateAgentRunPid(runId: UUID, pid: Int32) async {
        if let idx = agentRuns.firstIndex(where: { $0.id == runId }) {
            agentRuns[idx].pid = pid
            try? await persistence.saveAgentRuns(agentRuns)
        }
    }

    private func updateAgentRunStatus(runId: UUID, status: String, errorMsg: String? = nil) async {
        if let idx = agentRuns.firstIndex(where: { $0.id == runId }) {
            agentRuns[idx].status = status
            agentRuns[idx].completedAt = Date()
            if let errorMsg = errorMsg {
                try? await persistence.appendAgentRunLog(runId: runId, text: "\n=== Error: \(errorMsg) ===\n")
            } else {
