...
actor PersistenceService {
    ...
    func saveSession(_ session: ChatSession, persistTitle: Bool = false) async throws {
        ...
        case .cliBacked:
            // CLI owns the message log (jsonl). We only persist the JXCODE-only
    ...
    func loadLegacySessions(for projectId: UUID) -> [ChatSession.Summary] {
        ...
        return files
            ...
            .compactMap { url -> ChatSession.Summary? in
                ...
                // Files saved before this change may decode as `.legacyJXCODE`
                // already (default), but be defensive.
                if summary.origin != .legacyJXCODE {
    ...
    func deleteSession(projectId: UUID, sessionId: String, origin: SessionOrigin, cwd: String?) async throws {
        ...
        case .cliBacked:
            ...
            // Pre-cli-sync builds wrote a JXCODE-side json with the same sid. If
    ...
    /// - `.cliBacked` → CLI jsonl
    /// - `.legacyJXCODE` → JXCODE's per-project json
    ...
    func saveAgents(_ agents: [Agent]) throws {
        let url = baseURL.appendingPathComponent("agents.json")
        try encode(agents, to: url)
    }

    func loadAgents() -> [Agent] {
        let url = baseURL.appendingPathComponent("agents.json")
        return decode([Agent].self, from: url) ?? []
    }

    func saveAgentRuns(_ runs: [AgentRun]) throws {
        let url = baseURL.appendingPathComponent("agent_runs.json")
        try encode(runs, to: url)
    }

    func loadAgentRuns() -> [AgentRun] {
        let url = baseURL.appendingPathComponent("agent_runs.json")
        return decode([AgentRun].self, from: url) ?? []
    }

    func writeAgentRunLog(runId: UUID, text: String) throws {
        let dir = baseURL.appendingPathComponent("agent_runs")
        try ensureDirectory(dir)
        let url = dir.appendingPathComponent("\(runId.uuidString).log")
        try text.write(to: url, atomically: true, encoding: .utf8)
    }

    func appendAgentRunLog(runId: UUID, text: String) throws {
        let dir = baseURL.appendingPathComponent("agent_runs")
        try ensureDirectory(dir)
        let url = dir.appendingPathComponent("\(runId.uuidString).log")
    ...
    func readAgentRunLog(runId: UUID) -> String {
        let url = baseURL.appendingPathComponent("agent_runs").appendingPathComponent("\(runId.uuidString).log")
    ...
    func saveUsageRecords(_ records: [UsageRecord]) throws {
        let url = baseURL.appendingPathComponent("usage_records.json")
        try encode(records, to: url)
    }

    func loadUsageRecords() -> [UsageRecord] {
        let url = baseURL.appendingPathComponent("usage_records.json")
        return decode([UsageRecord].self, from: url) ?? []
    }
