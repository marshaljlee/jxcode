...
**The terminal was for the few. JXCODE is for everyone.**

JXCODE is a lightweight native macOS desktop client for Claude Code. It brings the CLI agent workflow into a project-centric GUI with streaming chat, repository switching, file browsing, Git status, permissions, terminal access, and per-project notes.
...
![JXCODE Screenshot](docs/screenshot.png)
...
JXCODE was built so my non-developer coworkers could use Claude Code without learning a shell first. It doesn't reinvent the agent. It spawns the real `claude` CLI underneath, so your `CLAUDE.md`, skills, MCP servers, and slash commands keep working as-is. What sits on top is a native Mac app:
...
| **Shortcut buttons** | Create quick buttons for prompts or terminal commands you run repeatedly. Terminal-command shortcuts can launch directly into JXCODE's interactive terminal popup. |
...
| **Notifications** | Optional system notifications with response previews while JXCODE is in the background. |
...
1. Download the latest `JXCODE-x.y.z.zip` from the [Releases](https://github.com/ttnear/JXCODE/releases) page.
2. Unzip and move `JXCODE.app` to your `Applications` folder.
3. Launch `JXCODE.app`.
...
When you see **"Apple could not verify 'JXCODE.app' is free of malware..."**:

1. Click **Done** on the dialog.
2. Open **System Settings -> Privacy & Security**.
3. Scroll to the Security section and click **Open Anyway** next to `JXCODE.app`.
4. Confirm with your password or Touch ID.

After this one-time approval, JXCODE launches normally. The app is signed with a Developer ID certificate and notarized by Apple. This prompt is standard macOS behavior, not a security warning specific to JXCODE.
...
```bash
open JXCODE.xcodeproj
```

For a CLI build:

```bash
xcodebuild -project JXCODE.xcodeproj -scheme JXCODE -configuration Debug build
```

For tests:

```bash
xcodebuild test -project JXCODE.xcodeproj -scheme JXCODE -destination 'platform=macOS'
...
| Path | Purpose |
|------|---------|
| `JXCODE/` | macOS app target: app entry point, views, services, resources, and integrations. |
| `Packages/Sources/JXCODECore/` | Shared models, theme, utilities, Git helpers, and pure core logic. |
| `Packages/Sources/JXCODEChatKit/` | Reusable chat UI, message rendering, input bar, slash commands, shortcuts, diffs, and status line. |
| `JXCODETests/` | App-level XCTest coverage. |
