...
struct EmptySessionView: View {
    ...
    let suggestions = [
        ("sparkles", "Write a clean feature", "Draft code conforming to modern architecture"),
        ("doc.text.magnifyingglass", "Analyze repository structure", "Explore paths, dependencies, and entrypoints"),
        ("bug", "Refactor and debug code", "Find memory leaks, slow logic, or lint errors"),
        ("testtube.2", "Generate unit tests", "Create test coverage for a specific component")
    ]
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                ForEach(suggestions, id: \.1) { icon, title, description in
                    Button {
                        windowState.inputText = title + " - " + description
                    } label: {
                        VStack(alignment: .leading, spacing: 6) {
                            HStack(spacing: 8) {
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 10)
                                .stroke(ClaudeTheme.textSecondary.opacity(0.12), lineWidth: 1)
                        )
                    }
                    .buttonStyle(.plain)
                    .contentShape(Rectangle())
