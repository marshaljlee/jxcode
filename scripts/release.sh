...
echo "▶ Starting JXCODE ${TAG} release"
echo ""

# ── 1. Bump version in pbxproj ───────────────
PBXPROJ="JXCODE.xcodeproj/project.pbxproj"
APPCAST="appcast.xml"
...
if [ -f "$META_FILE" ]; then
    ...
    DESC_BLOCK=""
    if [ "$HAS_NOTES" = "1" ]; then
        DESC_FILE="$(mktemp -t jxcode_desc).html"
...
${DESC_HTML}
    ...
    NEW_ITEM_FILE="$(mktemp -t jxcode_item).xml"
...
### Installation
1. Download \`JXCODE-${VERSION}.zip\`
2. Unzip and move \`JXCODE.app\` to \`/Applications\`
